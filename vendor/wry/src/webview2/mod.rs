...
impl InnerWebView {
  ...
  unsafe extern "system" fn parent_subclass_proc(
    ...
    // PARCHE LOCAL (backport de wry 0.56.1, ver vendor/wry/PARCHE.md):
    // si el controlador ya fue liberado, dwrefdata es nulo y los brazos de
    // WM_SIZE/WM_SETFOCUS/WM_MOVE lo desreferenciaban tal cual: panico no
    // desenrollable en el hilo principal al minimizar/restaurar (la ventana
    // muere y el audio sigue). Reproducido con scripts/harness-minimize.ps1.
    ...
    match msg {
      ...
      msg if msg == WM_DESTROY || msg == PARENT_DESTROY_MESSAGE => {
        // PARCHE LOCAL (backport de wry 0.56.1): el original re-registraba el
        // subclass con datos NULOS y cualquier mensaje posterior lo
        // desreferenciaba. Quitar el subclass elimina la mina.
